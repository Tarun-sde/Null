...
def test_complete_acceptance_flow(auth_client):
    # 1. Fetch recommendations
    recs = auth_client.get("/api/v1/recommendations").json()
    assert len(recs) > 0
    rec = recs[0]
    rec_id = rec["id"]
    ...
    # 3. Complete Action
    r_comp = auth_client.post(
        f"/api/v1/actions/{action_id}/complete",
        json={
            "notes": "Reassignment executed to Kallambella Wind Energy Corridor",
            "actor": "Commander Rajesh Sharma",
            "payload": {"target_site_id": "SITE-003"},
        },
    )
    ...
    r_dup = auth_client.post(f"/api/v1/actions/{action_id}/complete", json={})
    ...
    imp = auth_client.get(f"/api/v1/actions/{action_id}/impact").json()
    ...
    fleet_imp = auth_client.get("/api/v1/impact").json()
